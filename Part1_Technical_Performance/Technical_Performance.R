...
sample.info.100 <- read.table("../../batch/4 UMI_100/sample.info.txt")
sample.info.200 <- read.table("../../batch/5 UMI_200/sample.info.txt")
...
sample.info.4CL$Cell_Line <- factor(sample.info.4CL$Cell_Line,
                                    levels = c("GM12878","HepG2","Hela","H9","K562","293T",
                                               "AtT20","MEF","3T3"))
sample.info.100$Cell_Line <- factor(sample.info.100$Cell_Line,
                                    levels = c("GM12878","HepG2","Hela","H9","K562","293T",
                                               "AtT20","MEF","3T3"))
sample.info.200$Cell_Line <- factor(sample.info.200$Cell_Line,
                                    levels = c("GM12878","HepG2","Hela","H9","K562","293T",
                                               "AtT20","MEF","3T3"))
...
plot.data$Cell_Line <- factor(plot.data$Cell_Line, levels = plot.data$Cell_Line)
...
ggpubr::ggdonutchart(plot.data, "Freq", label = "label",                               
                     fill = "Cell_Line", color = "white", lab.pos = "in",
                     palette = c(color.cell,"unclassified" = "gray75"))
dev.off()  

sample.info.9CL <- sample.info.9CL[sample.info.9CL$Pass_QC == 1, ]
sample.info.9Mix <- sample.info.9Mix[sample.info.9Mix$Pass_QC == 1, ]
...
temp <- aggregate(sample.info.9CL$Mapped_percent,list(sample.info.9CL$Cell_Line),median)
p1 <- ggplot(sample.info.9CL)+
  geom_violin(aes(x=Cell_Line,y=Mapped_percent*100,fill=Cell_Line),alpha=0.7,show.legend = F)+
  geom_boxplot(aes(x=Cell_Line,y=Mapped_percent*100,color=Cell_Line),alpha=0,show.legend = F)+
  ylab("Mapping Rate (%)")+
  ...
  theme_lzy()+xlab(NULL)
temp <- aggregate(sample.info.9CL$Gene_Detected,list(sample.info.9CL$Cell_Line),median)
p2 <- ggplot(sample.info.9CL)+
  ...
  scale_x_discrete(breaks = temp$Group.1,
                   labels = paste0(temp$Group.1,"\n",signif(temp$x,4)))+
  theme_lzy()+xlab(NULL)
temp <- aggregate(sample.info.9CL$Trans_Detected,list(sample.info.9CL$Cell_Line),median)
...
aggregate(sample.info.9CL$Gene_Detected, by = list(Cell_Line = sample.info.9CL$Cell_Line), FUN=mean)
aggregate(sample.info.9CL$Trans_Detected, by = list(Cell_Line = sample.info.9CL$Cell_Line), FUN=mean)
aggregate(sample.info.9CL$Gene_Detected, by = list(Cell_Line = sample.info.9CL$Cell_Line), FUN=median)
aggregate(sample.info.9CL$Trans_Detected, by = list(Cell_Line = sample.info.9CL$Cell_Line), FUN=median)

aggregate(sample.info.4CL$Gene_Detected, by = list(Cell_Line = sample.info.4CL$Cell_Line), FUN=mean)
aggregate(sample.info.4CL$Trans_Detected, by = list(Cell_Line = sample.info.4CL$Cell_Line), FUN=mean)
aggregate(sample.info.4CL$Gene_Detected, by = list(Cell_Line = sample.info.4CL$Cell_Line), FUN=median)
aggregate(sample.info.4CL$Trans_Detected, by = list(Cell_Line = sample.info.4CL$Cell_Line), FUN=median)

## 1.2 accumulated frequency along expression level----------------
sensi.9CL <- myGeneSensitivity(rc_gene_9CL[,rownames(sample.info.9CL)])
sensi.4CL <- myGeneSensitivity(rc_gene_4CL[,rownames(sample.info.4CL)])
sensi.100 <- myGeneSensitivity(rc_gene_100[,rownames(sample.info.100)])
sensi.200 <- myGeneSensitivity(rc_gene_200[,rownames(sample.info.200)])
...
ggplot(sensi,aes(x = breaks, y = frequency*100,color=library))+
  geom_line(size=1)+
  geom_errorbar(aes(ymin=(frequency-sd)*100, ymax=(frequency+sd)*100),width=.1,size=0.5)+
...
ggplot(sample.info.merge)+
  ...
  geom_smooth(aes(x = Mapped_Reads / 1e5, y = Gene_Detected, color = "Gene"),
              method = "loess", se = T, span = 0.4,size =0.7)+
  geom_smooth(aes(x = Mapped_Reads / 1e5, y = Trans_Detected, color = "Isoform"),
              method = "loess", se = T, span = 0.7,size =0.7)+
...
ggplot(sample.info.merge)+
  ...
  #geom_point(aes(x = Mapped_Reads, y = Trans_Detected, color = "Isoform"))+
  geom_smooth(aes(x = Mapped_Reads/ 1e5, y = Gene_Detected, color = "Gene"),
              method = "loess", se = T, span = 0.8,size =0.7)+
  geom_smooth(aes(x = Mapped_Reads/ 1e5, y = Trans_Detected, color = "Isoform"),
              method = "loess", se = T, span = 0.8,size =0.7)+
...
mouse.trans <- rownames(rc_trans_4CL)[grep("ENSMUST",rownames(rc_trans_4CL))]
human.trans <- rownames(rc_trans_4CL)[-grep("ENSMUST",rownames(rc_trans_4CL))]
nrow(rc_trans_4CL) - length(mouse.trans) - length(human.trans)

Barnyard.4CL <- data.frame(row.names = colnames(rc_trans_4CL),
...
Barnyard.9CL <- data.frame(row.names = colnames(rc_trans_9CL),
...
Barnyard$ratio_human <- Barnyard$human_transcripts/(Barnyard$human_transcripts+
                                                      Barnyard$mouse_transcripts)
Barnyard$ratio_mouse <- Barnyard$mouse_transcripts/(Barnyard$human_transcripts+
                                                      Barnyard$mouse_transcripts)
...
n1 <- aggregate(Barnyard.4CL$human_transcripts, by = list(sample.info.4CL$Organism),sum)$x
n2 <- aggregate(Barnyard.4CL$mouse_transcripts, by = list(sample.info.4CL$Organism),sum)$x
n1 / (n1+n2) # human 0.98089715 
n2 / (n1+n2) # mouse 0.96737273

n1 <- aggregate(Barnyard.9CL$human_transcripts, by = list(sample.info.9CL$Organism),sum)$x
n2 <- aggregate(Barnyard.9CL$mouse_transcripts, by = list(sample.info.9CL$Organism),sum)$x
...
rc_Smart3 <- read.table("../../Test/Smart-Seq3/Smartseq3.HEK.fwdprimer.readcounts.txt")
uc_Smart3 <- read.table("../../Test/Smart-Seq3/Smartseq3.HEK.fwdprimer.UMIcounts.txt")
sample.info.Smart3 <- read.table("../../Test/Smart-Seq3/Smartseq3.HEK.fwdprimer.sample_annotation.txt",
                                 header = T)
table(rownames(rc_Smart3)%in%gene.info.hg38$`Gene stable ID`)

rc_SCAN2.1 <- rc_gene_4CL[,rownames(sample.info.4CL)[sample.info.4CL$Cell_Line=="293T"]]
rc_SCAN2.2 <- rc_gene_9CL[,rownames(sample.info.9CL)[sample.info.9CL$Cell_Line=="293T"]]
dim(rc_SCAN2.1)
dim(rc_SCAN2.2)
...
common.gene <- intersect(common.gene, gene.info.hg38$`Gene stable ID`)
names(common.gene) <- gene.info.hg38[common.gene,]$`Gene name`
common.gene <- common.gene[!duplicated(names(common.gene))]
common.gene <- common.gene[names(common.gene) %in% rownames(rc_SCAN2)]
...
plot.data <- data.frame("Smart3_Reads" = rowSums(rc_Smart3),
...
anno <- data.frame(row.names = colnames(rc_cmp),
                   "Group"=c(rep("SCANSeq2_4CL",170),
                             rep("SCANSeq2_9CL",80),
...
pheatmap(cor.gene.cmp, main = "Compare with Smart-Seq3, HEK293T",
...
# 9CL
anno <- data.frame(row.names = rownames(sample.info.9CL),
...
pheatmap(cor.gene.9CL, main = "Correlation at Gene Level",
         cluster_rows = F,cluster_cols = F,
         annotation_row = anno, annotation_col = anno,
         annotation_names_row = F, annotation_names_col = F,
...
for(one in unique(sample.info.9CL$Cell_Line)){
  temp <- data.frame("cell_line" = one,
                     "Cor" = as.numeric(cor.gene.9CL[sample.info.9CL[sample.info.9CL$Cell_Line==one,]$Rename,
                                                 sample.info.9CL[sample.info.9CL$Cell_Line==one,]$Rename]))
...
aggregate(cor.gene.9CL.melt$Cor, list(cor.gene.9CL.melt$cell_line), median)
...
# 4CL
anno <- data.frame(row.names = rownames(sample.info.4CL),
...
pheatmap(cor.gene.4CL, main = "Correlation at Gene Level",
         cluster_rows = F,cluster_cols = F,
         annotation_row = anno, annotation_col = anno,
         annotation_names_row = F, annotation_names_col = F,
         annotation_colors = list(cell_line = color.cell),
...
for(one in unique(sample.info.4CL$Cell_Line)){
  temp <- data.frame("cell_line" = one,
                     "Cor" = as.numeric(cor.gene.4CL[sample.info.4CL[sample.info.4CL$Cell_Line==one,]$Rename,
                                                     sample.info.4CL[sample.info.4CL$Cell_Line==one,]$Rename]))
...
aggregate(cor.gene.4CL.melt$Cor, list(cor.gene.4CL.melt$cell_line), median)
...
cor.gene.Smart3.melt <- cor.gene.Smart3.melt[cor.gene.Smart3.melt$Cor < 1,]
aggregate(cor.gene.Smart3.melt$Cor, list(cor.gene.Smart3.melt$cell_line), median)
# 293T Smart-Seq3 umi
uc_Smart3 <- read.table("../../Test/Smart-Seq3/Smartseq3.HEK.fwdprimer.UMIcounts.txt")
...
cor.gene.Smart3.umi.melt <- cor.gene.Smart3.umi.melt[cor.gene.Smart3.umi.melt$Cor < 1,]
aggregate(cor.gene.Smart3.umi.melt$Cor, list(cor.gene.Smart3.umi.melt$cell_line), median)


# compare
cor.gene.9CL.293T <- cor.gene.9CL.melt[cor.gene.9CL.melt$cell_line=="293T",]
...
plot.data <- rbind(cor.gene.9CL.293T, cor.gene.Smart3.melt,cor.gene.Smart3.umi.melt)
...
obj.9CL[["Isoform"]] <- CreateAssayObject(rc_trans_9CL[,colnames(obj.9CL)])
obj.9Mix[["Isoform"]] <- CreateAssayObject(rc_trans_9Mix[,colnames(obj.9Mix)])
...
PTPRC.iso.selected <- PTPRC.iso[!PTPRC.iso%in%c("PTPRC-215","PTPRC-216")]
...
dev.off()
rm(obj.combined)
rm(rc_gene_qc, rc_trans_4CL, rc_trans_9CL, rc_gene_100, rc_gene_200, rc_gene_4CL, rc_gene_9CL)
...
gtf <- rtracklayer::import("IsoformDistinction/Homo_sapiens.GRCh38.101.PTPRC.gtf")
gtf=as.data.frame(gtf)

PTPRC_exons <- gtf %>% dplyr::filter(type == "exon")
temp <- PTPRC_exons[PTPRC_exons$transcript_biotype == "protein_coding",]
temp <- temp[!duplicated(temp$exon_id),]
temp$transcript_name <- "gene_PTPRC"
PTPRC_exons <- rbind(PTPRC_exons,temp)
PTPRC_exons.select <- PTPRC_exons[PTPRC_exons$transcript_name%in%c(c("gene_PTPRC",
                                                                     "PTPRC-201",
                                                                     "PTPRC-203",
                                                                     "PTPRC-209")),]
PTPRC_exons.select$transcript_name <- factor(PTPRC_exons.select$transcript_name,
                                             levels = rev(c("gene_PTPRC",
                                                        "PTPRC-201",
                                                        "PTPRC-203",
                                                        "PTPRC-209")))
...
temp %>%
  ...
  geom_range(aes(fill = transcript_biotype),show.legend = F,size = 0) +
...
PTPRC_exons.select %>%
  ...
  geom_intron(data = to_intron(PTPRC_exons.select, "transcript_name"),aes(strand = strand))+
...
sample.info.merge <- rbind(sample.info.9CL,
                           sample.info.4CL,
                           sample.info.9Mix,
                           sample.info.100,
                           sample.info.200)
