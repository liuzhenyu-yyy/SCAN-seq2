...
sample.info[(as.numeric(sample.info$Barcode.5)%in%(c(54:56,72:74, 75,76)) &
               as.numeric(sample.info$Barcode.3)%in%(c(01:16))) | 
              (as.numeric(sample.info$Barcode.5)%in%(c(51:53,69:71,78)) &
                 as.numeric(sample.info$Barcode.3)%in%(c(17:32))),]$Cell_Line <- "293T"
sample.info[(as.numeric(sample.info$Barcode.5)%in%(c(51:53,63:65,78,79)) &
               as.numeric(sample.info$Barcode.3)%in%(c(01:16))) | 
              (as.numeric(sample.info$Barcode.5)%in%(c(54:56, 66:68,80,75)) &
                 as.numeric(sample.info$Barcode.3)%in%(c(17:32))),]$Cell_Line <- "3T3"
sample.info[(as.numeric(sample.info$Barcode.5)%in%(c(57:59,69:71,80)) &
               as.numeric(sample.info$Barcode.3)%in%(c(01:16))) | 
              (as.numeric(sample.info$Barcode.5)%in%(c(60:62,72:74,76,77)) &
                 as.numeric(sample.info$Barcode.3)%in%(c(17:32))),]$Cell_Line <- "MEF"
sample.info[(as.numeric(sample.info$Barcode.5)%in%(c(60:62,66:68,77)) &
               as.numeric(sample.info$Barcode.3)%in%(c(01:16))) | 
              (as.numeric(sample.info$Barcode.5)%in%(c(57:59,63:65,79,80)) &
                 as.numeric(sample.info$Barcode.3)%in%(c(17:32))),]$Cell_Line <- "K562"
...
ggplot()+
  geom_point(aes(x=log10(1+ERCC.info$concentration_mix_1),
                 y=log10(1+rc_ERCC$`4CL_Bc51_Bc01`),color="Cell1"))+
  geom_point(aes(x=log10(1+ERCC.info$concentration_mix_1),
                 y=log10(1+rc_ERCC$`4CL_Bc51_Bc07`),color="Cell2"))+
  geom_point(aes(x=log10(1+ERCC.info$concentration_mix_1),
                 y=log10(1+rc_ERCC$`4CL_Bc51_Bc09`),color="Cell3"))+
  geom_point(aes(x=log10(1+ERCC.info$concentration_mix_1),
                 y=log10(1+rc_ERCC$`4CL_Bc51_Bc15`),color="Cell4"))+
...
write.table(sample.info[sample.info$Organism=="Human",]$Name,
            "sample.list.hg38",quote = F,sep = "\t",row.names = F,col.names = F)
write.table(sample.info[sample.info$Organism=="Mouse",]$Name,
            "sample.list.mm10",quote = F,sep = "\t",row.names = F,col.names = F)
sample.info.qc <- sample.info[sample.info$Pass_QC==1,]
for (one in levels(sample.info.qc$Cell_Line)) {
  write.table(sample.info.qc[sample.info.qc$Cell_Line==one,]$Name,
              paste0("sample.list.4CL.",one),quote = F,sep = "\t",row.names = F,col.names = F)
}
