...
marker.list <- c("IGHM","ALB","LY6K", # GM12878, HepG2, Hela
                 "CD3D","HBA1","BEX3", # H9, K562, 293T
                 "Apobec3","Col1a1","Lgals3" # mES, 3T3
)
...
# cell type
p <- DimPlot(mix.obj, reduction = "umap",label = T,pt.size = 0.6)+
  theme_bw()+theme(panel.grid = element_blank())
...
mix.obj@meta.data$Cell_Line <- factor(mix.obj@meta.data$Cell_Line,
...
# 5. Barnyard plot --------------------------------------------------------
sample.info[colnames(mix.obj),]$Cell_Line <- as.character(mix.obj$Cell_Line)
table(sample.info$Cell_Line)
...
ggplot()+
  geom_point(aes(x=log10(1+ERCC.info$concentration_mix_1),
                 y=log10(1+rc_ERCC$`9CL_Mix_Bc33_Bc01`),color="Cell1"))+
  geom_point(aes(x=log10(1+ERCC.info$concentration_mix_1),
                 y=log10(1+rc_ERCC$`9CL_Mix_Bc33_Bc09`),color="Cell2"))+
  geom_point(aes(x=log10(1+ERCC.info$concentration_mix_1),
                 y=log10(1+rc_ERCC$`9CL_Mix_Bc33_Bc32`),color="Cell3"))+
  geom_point(aes(x=log10(1+ERCC.info$concentration_mix_1),
                 y=log10(1+rc_ERCC$`9CL_Mix_Bc34_Bc01`),color="Cell4"))+
...
for (one in unique(sample.info.qc$Cell_Line)) {
  write.table(sample.info.qc[sample.info.qc$Cell_Line==one,]$Name,
              paste0("sample.list.9Mix.",one),quote = F,sep = "\t",row.names = F,col.names = F)
}
