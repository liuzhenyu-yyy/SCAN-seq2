...
for (one in 1:4) {
  ...
  model <-
    lm(log10(plot.data.sub$SCANSeq2 + 1) ~ log10(plot.data.sub$Smart3_Reads +
                                                   1))

  cor.value <-
    cor(log10(plot.data.sub$Smart3_Reads + 1),
        log10(plot.data.sub$SCANSeq2 + 1))
