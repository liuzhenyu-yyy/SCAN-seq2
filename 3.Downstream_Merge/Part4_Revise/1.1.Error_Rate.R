...
barcode <- readDNAStringSet(
  "data/Barcode01_96.fa",
  format = "fasta",
...
for (cell in rownames(sample.info)) {
  message(paste0(
    Sys.time(),
    ": Calculating barcode mismatch in cell ",
    cell,
    "..."
  ))
  ...
  reads <-
    readDNAStringSet(
      paste0("data/subset.fq/", cell, "_sub5000_clean.fq.gz"),
      format = "fastq",
      seek.first.rec = FALSE,
      use.names = F
    )
  ...
  for (one in 1:1000) {
    agrep.5p <- adist(barcode[[sample.info[cell, ]$Barcode.5]],
                      ...
                      ignore.case = T)
    agrep.3p <- adist(
      barcode[[sample.info[cell, ]$Barcode.3]],
      partial = T,
      reads[[one]],
      counts = T,
      ignore.case = T
    ...
    align.5p <-
      pairwiseAlignment(
        barcode[[sample.info[cell, ]$Barcode.5]],
        reads[[one]],
    ...
    if (align.5p@score > 10) {
      ...
      nIndel_5p <-
        c(nIndel_5p,
          nindel(align.5p)@insertion[1, 1] +  nindel(align.5p)@deletion[1, 1])
    }
    ...
    if (align.3p@score > 10) {
      ...
      nIndel_3p <-
        c(nIndel_3p,
          nindel(align.3p)@insertion[1, 1] +  nindel(align.3p)@deletion[1, 1])
    }
  }
...
multiplot(
  ggplot(sample.info, aes(
    x = gsub("Barcode", "", Barcode.5), y = Mean_Mismatch_5p
  )) +
    geom_boxplot(outlier.size = 0.5) + facet_wrap( ~ "Mismatch") + theme_lzy() +
    theme(axis.title.x = element_blank()) + ylab("nMismatch"),
  ggplot(sample.info, aes(
    x = gsub("Barcode", "", Barcode.5), y = Mean_Indel_5p
  )) +
    geom_boxplot(outlier.size = 0.5) + facet_wrap( ~ "Indel") + theme_lzy() +
    theme(axis.title.x = element_blank()) + ylab("nIndel"),
  ggplot(sample.info, aes(
    x = gsub("Barcode", "", Barcode.5),
    y = Mean_Mismatch_5p + Mean_Indel_5p
  )) +
    geom_boxplot(outlier.size = 0.5) + facet_wrap( ~ "Overall Error") + theme_lzy() +
...
gene2trans <-
  read.table("data/TranscriptID_GeneID_from_cDNA_fasta.txt")
colnames(gene2trans) <- c("Trans_id", "Gene_id")
rownames(gene2trans) <- gene2trans$Trans_id

UMI_NGS <-
  as.data.frame(data.table::fread("data/NGS-UMI-NGS-UMI-1.NGS.UMIs.txt",
                                  header = F))
UMI_NGS$Method <- "NGS"
UMI_TGS <-
  as.data.frame(data.table::fread("data/M_4CL.TGS.UMIs.txt",
                                  header = F))
UMI_TGS$Method <- "TGS"
...
for (one in ERCC) {
  ...
  message(paste0(
    paste0(
      ...
      ": ",
      length(seq_TGS),
      " UMIs detected by TGS, spliting into ",
      length(seq_TGS_split),
      " trunks."
    )
  ))
  ...
  for (trunk in names(seq_TGS_split)) {
    message(paste0(paste0(
      Sys.time(), ": Processing trunk ", trunk, "..."
    )))
...
# overlap
UMI_TGS.ERCC$ID_corrected <-
  paste(UMI_TGS.ERCC$Gene_id, UMI_TGS.ERCC$Min_UMI, sep = "_")
