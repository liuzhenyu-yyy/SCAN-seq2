...
sample.info.TGS <- read.table("data/TGS/summary.txt",header = T, row.names = 1)

sample.info.TGS$Library <- "SCAN2-IGG"
sample.info.TGS$Name <- rownames(sample.info.TGS)
sample.info.TGS$Rename <- paste("SCAN2-IGG", sample.info.TGS$Name, sep = "_")
...
sample.info.TGS[sample.info.TGS$Barcode.5 %in% 
                  as.character(c(54:56,63:65,72:74)),]$Concentration <- 10
sample.info.TGS[sample.info.TGS$Barcode.5 %in% 
                  as.character(c(57:59,66:68,75)),]$Concentration <- 30
...
rc.TGS <- rc.TGS[rownames(rc.TGS) %in% gene.info.hg38$`Gene stable ID`,]
rownames(gene.info.hg38) <- gene.info.hg38$`Gene stable ID`
rc.TGS <- rc.TGS[!duplicated(gene.info.hg38[rownames(rc.TGS),]$`Gene name`),]
...
for (one in samples) {
  ...
  sample.info <- read.table(paste0("data/NGS/",one, ".gene_tran_num.xls"), header = T)
  ...
  rc <- read.table(paste0("data/NGS/",one, ".umi_counts.xls"), header = T, row.names = 1)
...
sample.info.NGS[grep("2|5|8", sample.info.NGS$Library),]$Concentration <- 10
sample.info.NGS[grep("3|6|9", sample.info.NGS$Library),]$Concentration <- 30
...
TGS.obj$percent.mt <- PercentageFeatureSet(TGS.obj, pattern = "^MT-|^mt-")
NGS.obj$percent.mt <- PercentageFeatureSet(NGS.obj, pattern = "^MT-|^mt-")

pdf("Scatter.QC.pdf", 10, 4)
FeatureScatter(TGS.obj, feature1 = "nGenes",
               feature2 = "percent.mt", group.by = "Concentration") + ggtitle("SCAN-seq2") +
  FeatureScatter(NGS.obj, feature1 = "nGenes",
                 feature2 = "percent.mt", group.by = "Concentration") + ggtitle("STRT-seq")
FeatureScatter(TGS.obj, feature1 = "nGenes",
               feature2 = "percent.mt", group.by = "Time") + ggtitle("SCAN-seq2") +
  FeatureScatter(NGS.obj, feature1 = "nGenes",
                 feature2 = "percent.mt", group.by = "Time") + ggtitle("STRT-seq")
dev.off()

TGS.obj <- subset(TGS.obj, subset = nGenes > 2500 & percent.mt < 15) # 579 / 793
NGS.obj <- subset(NGS.obj, subset = nGenes > 2500 & percent.mt < 15) # 581 / 816
...
TGS.obj <- RunUMAP(TGS.obj, dims = 1:5,
                   n.neighbors = 50, min.dist = 0.1)
NGS.obj <- RunUMAP(NGS.obj, dims = 1:5,
                   n.neighbors = 50, min.dist = 0.1)
...
TGS.obj <- CellCycleScoring(TGS.obj,
                            s.features = cc.genes.updated.2019$s.genes,
                            g2m.features = cc.genes.updated.2019$g2m.genes,
                            set.ident = T)
NGS.obj <- CellCycleScoring(NGS.obj,
                            s.features = cc.genes.updated.2019$s.genes,
                            g2m.features = cc.genes.updated.2019$g2m.genes,
                            set.ident = T)
...
TGS.obj <- ScaleData(TGS.obj,
                     vars.to.regress = c("S.Score", "G2M.Score"),
                     features = rownames(TGS.obj)) %>%
  RunPCA(npcs = 30) %>%
  FindNeighbors() %>%
  RunUMAP(dims = 1:5, n.neighbors = 50, min.dist = 0.1)
NGS.obj <- ScaleData(NGS.obj,
                     vars.to.regress = c("S.Score", "G2M.Score"),
