...
classification <-
  read.table("data/GM12878.c10.SQANTI3_classification.txt", header = T)
...
NIC <-
  classification$isoform[classification$structural_category == "novel_in_catalog"]
...
classification$subcategory <-
  gsub("combination_of_known_junctions",
       "CJ",
       classification$subcategory)
classification$subcategory <-
  gsub("combination_of_known_splicesites",
       "CS",
       classification$subcategory)
...
report$nCell <-
  unlist(lapply(strsplit(report$sources, ','), FUN = length))

GM12878.bed.out <- GM12878.bed
GM12878.bed.out$V4 <- paste0(GM12878.bed.out$V4,
                             "_",
                             classification$subcategory,
                             "_",
                             report$nCell)
...
classification <-
  read.table("data/H9.SQANTI3_classification.txt", header = T)
...
H9.bed.out <- H9.bed
H9.bed.out$V4 <- paste0(H9.bed.out$V4,
                        "_",
                        classification$subcategory,
                        "_",
                        report$nCell)
...
gene.selected <-
  pseudogenes$`Gene name`[pseudogenes$`Gene name` %in% rownames(bulk.express)
                          &
                            pseudogenes$Parent %in% rownames(bulk.express)]

## 2.1. GM12878 -----
pseudogenes.GM <-
  pseudogenes[pseudogenes$`Gene name` %in% gene.selected, ]
rownames(pseudogenes.GM) <- pseudogenes.GM$`Gene name`
pseudogenes.GM$Expressed_pgene <- NULL
pseudogenes.GM$Expressed_parent <- NULL
pseudogenes.GM$Expression_pgene <-
  bulk.express[pseudogenes.GM$`Gene name`, ]$GM12878
pseudogenes.GM$Expression_parent <-
  bulk.express[pseudogenes.GM$Parent, ]$GM12878

pseudogenes.GM <- pseudogenes.GM[pseudogenes.GM$Expression_pgene +
                                   pseudogenes.GM$Expression_parent > 0, ]
pseudogenes.GM$log2_fc <-
  log2((pseudogenes.GM$Expression_pgene + 0.01) /
         (pseudogenes.GM$Expression_parent + 0.01))
write.table(
  pseudogenes.GM,
  "pseudogenes.GM12878.tsv",
  sep = "\t",
  row.names = F,
  col.names = T,
  quote = F
...
pseudogenes.H9$Expression_pgene <-
  bulk.express[pseudogenes.H9$`Gene name`, ]$H9
pseudogenes.H9$Expression_parent <-
  bulk.express[pseudogenes.H9$Parent, ]$H9

pseudogenes.H9 <- pseudogenes.H9[pseudogenes.H9$Expression_pgene +
                                   pseudogenes.H9$Expression_parent > 0, ]
pseudogenes.H9$log2_fc <-
  log2((pseudogenes.H9$Expression_pgene + 0.01) /
...
obj.9CL[["Isoform"]] <-
  CreateAssayObject(rc_trans[, colnames(obj.9CL)])
obj.9Mix[["Isoform"]] <-
  CreateAssayObject(rc_trans[, colnames(obj.9Mix)])
...
obj.combined <-
  subset(obj.combined, subset = Cell_Line %in% c("GM12878", "H9"))
...
DTU <- posthoc_and_stager(dturtle = DTU,
                          ofdr = 0.05,
                          posthoc = 0.1)
...
## 3.4. output table ---------------------------------------------------------
bulk.gene <-
  AverageExpression(object = obj.combined, assays = "Gene")
bulk.isoform <-
  AverageExpression(object = obj.combined, assays = "Isoform")
...
out$Percent_GM12878 <-
  out$Expression_GM12878 / out$Expression_Gene_GM12878
...
# House Keeping
temp <-
  as.data.frame(bulk.gene$Gene %>% as.data.frame %>% .[.$H9 > 0 &
                                                         .$GM12878 > 0,])
