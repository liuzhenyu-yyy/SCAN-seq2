...
obj.4CL <-
  readRDS("E:/LabWork/Project/SCANSeq2/batch/3 4CL/obj.4CL.rds")

obj.combined <- merge(
  obj.9CL,
  y = c(obj.9Mix, obj.4CL),
  add.cell.ids = c("9CL", "9Mix", "4CL"),
  project = "SCAN_Seq2"
)
obj.combined$Organism <-
  ifelse(
    obj.combined$Cell_Line %in% c("GM12878", "HepG2", "Hela", "H9", "K562", "293T"),
    "Human",
    "Mouse"
  )
...
multiplot(
  DimPlot(obj.combined, group.by = "Cell_Line", cols = color.cell) +
...
multiplot(
  DimPlot(obj.human, group.by = "Cell_Line", cols = color.cell) +
...
tpm <-
  as.data.frame(apply(rc, 2, function(x) {
    return(x * 1e6 / sum(x))
  }))
colSums(tpm)
rm(rc)
...
similarity <-
  read.table("E:/LabWork/genome/pseudogene/similarity.dat.txt", header = T)
...
similarity.sub <-
  similarity[similarity$parent %in% trans.info.hg38$`Transcript stable ID`, ]
similarity.sub <-
  similarity.sub[similarity.sub$pgene %in% trans.info.hg38$`Transcript stable ID`, ]
...
features <- c("RPS2",
              ...
              p.align = grid.grabExpr({
                ...
                spiral_text(
                  ...
                  query_alt,
                  facing = "inside",
                  gp = gpar(fontsize = 8, col = col.query[query_alt]),
                  just = "top",
                )
                spiral_text(
                  1:n - 0.5,
                  ...
                  subject_alt,
                  facing = "inside",
                  gp = gpar(fontsize = 8, col = col.subject[subject_alt]),
                  just = "bottom"
                )
              ...
              pseudogenes <-
                gene.info.hg38[grep("pseudogene", gene.info.hg38$`Gene type`), ]
              table(pseudogenes$`Gene type`)
              pseudogenes <-
                pseudogenes[grep("IG_|TR_|rRNA_", pseudogenes$`Gene type`, invert = T), ]
              table(pseudogenes$`Gene type`)
              pseudogenes$pseudo_type <-
                gsub("transcribed_|translated_", "", pseudogenes$`Gene type`)
              table(pseudogenes$pseudo_type)

              View(pseudogenes[pseudogenes$pseudo_type == "pseudogene", ])
              pseudogenes <-
                pseudogenes[pseudogenes$pseudo_type != "pseudogene", ] # 14501 ps
              table(pseudogenes$pseudo_type)
              ...
              for (i in breaks.tpm) {
                for (j in breaks.cell) {
                  ...
                  nCells <-
                    aggregate(
                      nCells,
                      by = list("Type" = pseudogenes$pseudo_type),
                      FUN = function(x) {
                        return(sum(x > j))
                      }
                    )
              ...
              ps.expressed$Mean_TPM <- apply(tpm.ps[ps.expressed$`Gene name`,],
                                             1,
                                             function(x) {
                                               return(mean(head(sort(x, decreasing = T), 100)))
                                             })
              ps.expressed$Expression_Level <- "none"
              ps.expressed[ps.expressed$Mean_TPM >= 0, ]$Expression_Level <-
                "0-10"
              ps.expressed[ps.expressed$Mean_TPM >= 10, ]$Expression_Level <-
                "10-50"
              ps.expressed[ps.expressed$Mean_TPM >= 50, ]$Expression_Level <-
                "50-100"
              ps.expressed[ps.expressed$Mean_TPM >= 100, ]$Expression_Level <-
                "100-500"
              ps.expressed[ps.expressed$Mean_TPM >= 500, ]$Expression_Level <-
                ">500"
              table(ps.expressed$Expression_Level)
              ps.expressed$Expression_Level <-
                factor(ps.expressed$Expression_Level,
                       levels = rev(c(
                         "0-10", "10-50", "50-100", "100-500", ">500"
                       )))

              plot.data <-
                as.data.frame(table(ps.expressed$pseudo_type, ps.expressed$Expression_Level))
              colnames(plot.data) <- c("Type", "TPM", "Count")
              plot.data$Type <- gsub("_pseudogene", "", plot.data$Type)
              plot.data$Type <-
                factor(plot.data$Type,
                       levels = c("processed", "unprocessed",
                                  "unitary", "polymorphic"))
              plot.data$Percent <-
                plot.data$Count / rep(table(ps.expressed$pseudo_type)) * 100

              pdf("Bar.Expression.level.pdf", 8, 4)
              multiplot(
                ggplot(plot.data) +
                  geom_bar(aes(
                    x = Type, y = Count, fill = TPM
                  ), stat = "identity") +
                  theme_lzy() + theme(axis.text.x = element_text(angle = 60, hjust = 1)) +
                  scale_fill_manual(values = rev(brewer.pal(6, "Reds")[1:5])),
                ggplot(plot.data) +
                  geom_bar(aes(
                    x = Type, y = Percent, fill = TPM
                  ), stat = "identity") +
                  theme_lzy() + theme(axis.text.x = element_text(angle = 60, hjust = 1)) +
                  scale_fill_manual(values = rev(brewer.pal(6, "Reds")[1:5])),
                cols = 2
              ...
              VlnPlot(
                object = obj.combined,
                c("AC092053.1", "TPTEP1", "C19orf48", "GPX1"),
                cols = color.cell,
                ncol = 4
              )
              dev.off()
              ...
              ps.parent <-
                as.data.frame(
                  fread(
                    "E:/LabWork/genome/pseudogene/pseudogene.parent.ensembl101.tsv",
                    sep = "\t"
                  )
                )
              ...
              pseudogenes[pseudogenes$Parent == pseudogenes$`Gene name`, ]$Parent <-
                "none"
              rm(ps.parent)
              length(unique(pseudogenes$Parent)) # 3256 - 1

              # pgene
              pseudogenes$Expressed_pgene <-
                ifelse(pseudogenes$`Gene name` %in% ps.expressed$`Gene name`,
                       1, 0)
              ...
              pseudogenes$Expressed_parent <-
                ifelse(pseudogenes$Parent %in% parents,
                       1, 0)
              ...
              ps.coexpress <-
                pseudogenes[pseudogenes$Expressed_pgene == 1 &
                              pseudogenes$Expressed_parent == 1, ]
              table(ps.coexpress$pseudo_type)

              ## 3.2. compare expression level of pgene and parent
              ps.coexpress$Mean_TPM_Pgene <-
                apply(tpm.ps[ps.coexpress$`Gene name`,],
                      1,
                      function(x) {
                        return(mean(head(sort(x, decreasing = T), 100)))
                      })
              ...
              ggplot(ps.coexpress[ps.coexpress$pseudo_type %in% c("processed_pseudogene", "unprocessed_pseudogene"), ]) +
                geom_histogram(
                  aes(
                    x = log10(Mean_TPM_Pgene / Mean_TPM_Parent),
                    fill = pseudo_type
                  ),
                  show.legend = F,
                  alpha = 0.5,
              ...
              for (one in 1:nrow(ps.coexpress)) {
                test <-
                  cor.test(
                    x = as.numeric(tpm[ps.coexpress[one, ]$`Gene name`, ]),
              ...
              ps.coexpress[ps.coexpress$Spearman_cor > 0.2, ]$Correltaion <-
                "Positive"
              ps.coexpress[ps.coexpress$Spearman_cor < (-0.2), ]$Correltaion <-
                "Negative"
              ...
              ps.coexpress$label <-
                paste(ps.coexpress$Parent, ps.coexpress$`Gene name`, sep = "_")
              ps.coexpress[ps.coexpress$Correltaion == "not significant", ]$label <-
                NA
              ...
              ggplot(ps.coexpress, aes(x = Spearman_cor, y = -log10(p.adjust))) +
                ...
                ggrepel::geom_label_repel(
                  aes(label = label, color = Correltaion),
                  box.padding = 0.1,
                  label.padding = 0.1,
                  label.r = 0.05,
                  label.size = 0.1,
              ...
              ps.pos <-
                ps.coexpress[ps.coexpress$Correltaion == "Positive", ]$`Gene stable ID`
              ps.pos <- ps.pos[order(ps.coexpress[ps.pos, ]$Spearman_cor)]
              ps.neg <-
                ps.coexpress[ps.coexpress$Correltaion == "Negative", ]$`Gene stable ID`
              ...
              for (one in ps.pos) {
                plot.data <-
                  data.frame(
                    "Parent" = as.numeric(tpm[ps.coexpress[one, ]$Parent, ]),
              ...
              obj.human.ps <-
                CreateSeuratObject(rc_ps, meta.data = obj.human@meta.data)
              ...
              top10 <-
                markers.ps %>% group_by(cluster) %>% top_n(n = 5, wt = avg_log2FC)
              table(top10$cluster)
