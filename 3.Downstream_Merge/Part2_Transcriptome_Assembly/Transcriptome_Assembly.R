...
sample.info <-
  rbind(sample.info.9CL, sample.info.9Mix, sample.info.4CL)
sample.info$Library <-
  factor(sample.info$Library, levels = c("9CL", "9CL_Mix", "4CL"))
...
isoform.count.9CL <-
  read.table("../../batch/1 9CL/data/Isoform.category.tsv")
isoform.count.9Mix <-
  read.table("../../batch/2 9CL_Mix/data/Isoform.category.tsv")
isoform.count.4CL <-
  read.table("../../batch/3 4CL/data/Isoform.category.tsv")
colnames(isoform.count.9CL) <-
  c(
    "Name",
    "Isoforms",
    "FSM",
...
isoform.count <-
  rbind(isoform.count.9CL, isoform.count.9Mix, isoform.count.4CL)
...
ggplot(sample.info) +
  geom_violin(
    aes(x = Cell_Line, y = Isoforms / 1000, fill = Cell_Line),
    alpha = 0.7,
    show.legend = F
  ) +
  geom_boxplot(
    aes(x = Cell_Line, y = Isoforms / 1000, color = Cell_Line),
    alpha = 0,
    show.legend = F
  ) +
...
for (one in features) {
  p <- ggplot(sample.info) +
    geom_violin(
      aes(x = Cell_Line, y = sample.info[, one] * 100, fill = Cell_Line),
      alpha = 0.7,
      show.legend = F
    ) +
    geom_boxplot(
...
for (one in names(color.cell)) {
  temp <-
    as.data.frame(fread(paste0(
      "data/cell_line/", one, "_trans_report.txt"
    )))
  temp$Cell_Line <- one
...
temp <-
  unlist(lapply(strsplit(assembly.CL.bed$sources, ','), FUN = length))
...
for (one in names(color.cell)) {
  ...
  temp <-
    unlist(lapply(breaks, function(x) {
      return(sum(nCells >= x))
    }))
  nIsoform_one <- data.frame(
    "Threshold" = breaks,
    "nIsoform" = temp,
    "Cell_Line" = rep(one)
  )
...
ggplot(nIsoform[nIsoform$Threshold <= 20, ], aes(x = Threshold, y = nIsoform / 1000, color = Cell_Line)) +
  ...
  theme_classic()
ggplot(nIsoform[nIsoform$Threshold <= 40, ], aes(
  x = Threshold,
  y = log10(nIsoform),
  color = Cell_Line
)) +
...
assembly.CL.filtered.bed <-
  assembly.CL.bed[assembly.CL.bed$num_cells >= 10, ]
...
for (one in names(color.cell)) {
  ...
  trans.id <- unlist(lapply(trans.id, function(x) {
    return(x[2])
  }))
  ...
  trans.select <-
    assembly.CL.filtered.bed[assembly.CL.filtered.bed$Cell_Line == one, ]$transcript_id
  bed <- bed[trans.id %in% trans.select, ]
  write.table(
    bed,
    paste0("data/cell_line/", one, ".c10.bed"),
    row.names = F,
    col.names = F,
    sep = "\t",
    quote = F
...
isoform.mouse <-
  read.table(
    "data/merge/merge_mouse_3CL.SQANTI3_classification.txt",
    header = T,
    stringsAsFactors = F
  )
temp <- read.table(
  "data/merge/merge_mouse_3CL_trans_report.txt",
  header = T,
  stringsAsFactors = F
)
...
isoform.human <-
  read.table(
    "data/merge/merge_human_6CL.SQANTI3_classification.txt",
    header = T,
    stringsAsFactors = F
  )
temp <- read.table(
  "data/merge/merge_human_6CL_trans_report.txt",
  header = T,
  stringsAsFactors = F
)
...
isoform.merge$structural_category <- factor(
  temp,
  levels = c(
    "FSM",
    "ISM",
...
isoform.merge$nCell_Line <-
  as.numeric(unlist(lapply(
    strsplit(isoform.merge$source, ","), length
  )))
...
plot.data <- as.data.frame(
  table(
    isoform.merge$structural_category,
    isoform.merge$Organism,
    isoform.merge$nCell_Line
  )
)
colnames(plot.data) <-
  c("structural_category", "Organism", "nCell_Line", "Frequency")
plot.data$nCell_Line <-
  factor(as.numeric(plot.data$nCell_Line), levels = 6:1)
...
# junctions
table(junctions.human$canonical,
      junctions.human$junction_category)


## 3.2. detailed classification of NIC -------------------------------------
isoform.merge.NIC <-
  isoform.merge[isoform.merge$structural_category == "NIC",]
...
plot.data <-
  as.data.frame(table(isoform.merge.NIC[isoform.merge.NIC$Organism == "Human", ]$subcategory))
...
plot.data <-
  as.data.frame(table(isoform.merge.NIC[isoform.merge.NIC$Organism == "Mouse", ]$subcategory))
plot.data$label <- paste0(plot.data$Var1, "\n",
                          signif(plot.data$Freq / sum(plot.data$Freq) *
                                   100, 3), "%")
ggpubr::ggdonutchart(
  plot.data,
  "Freq",
...
IG_C_genes <-
  sort(gene.info.hg38[gene.info.hg38$`Gene type` == "IG_C_gene", ]$`Gene name`)
...
PlotVDJDonut <- function(data, col) {
  ...
  plot.data$Color <- factor(plot.data$Color,
                            levels = rev(unique(rev(
                              c(plot.data$Color, "none")
                            ))))

  colors <-
    rep(brewer.pal(8, "Pastel2"), 3)[1:length(plot.data$Color)]
...
for (one in cell.list) {
  if (file.exists(paste0("data/bcr/", one, ".IgBlast.IGH.tsv"))) {
    temp <-
      read.table(
        paste0("data/bcr/", one, ".IgBlast.IGH.tsv"),
        header = T,
        sep = "\t"
      )
    #temp <- VDJFilter(temp)
  ...
  if (file.exists(paste0("data/bcr/", one, ".IgBlast.IGL.tsv"))) {
    temp <-
      read.table(
        paste0("data/bcr/", one, ".IgBlast.IGL.tsv"),
        header = T,
        sep = "\t"
      )
    #temp <- VDJFilter(temp)
...
VDJ.IGH[(nrow(VDJ.IGH) + 1):(nrow(VDJ.IGH) + length(empty)), ] <-
  "none"
VDJ.IGH[(nrow(VDJ.IGH) - length(empty) + 1):(nrow(VDJ.IGH)), ]$Cell <-
  empty
empty <- cell.list[!cell.list %in% VDJ.IGL$Cell]
VDJ.IGL[(nrow(VDJ.IGL) + 1):(nrow(VDJ.IGL) + length(empty)), ] <-
  "none"
VDJ.IGL[(nrow(VDJ.IGL) - length(empty) + 1):(nrow(VDJ.IGL)), ]$Cell <-
  empty
...
colnames(BCR.stat) <-
  c("Cell", "IGL_Reads", "IGH_Reads", "IGL_Cluster", "IGH_Cluster")
rownames(BCR.stat) <- BCR.stat$Cell
identical(BCR.stat$Cell, VDJ.IGL$Cell)
identical(BCR.stat$Cell, VDJ.IGH$Cell)
VDJ.IGH[BCR.stat$IGH_Reads < 5, c("v_call", "d_call", "j_call")] <-
  "none"
VDJ.IGL[BCR.stat$IGL_Reads < 5, c("v_call", "d_call", "j_call")] <-
  "none"
...
VDJ.IGH.sub[VDJ.IGH.sub$subclone == subclones[1], ]$subclone <-
  names(subclones)[1]
VDJ.IGH.sub[VDJ.IGH.sub$subclone == subclones[2], ]$subclone <-
  names(subclones)[2]
VDJ.IGH.sub[VDJ.IGH.sub$subclone == subclones[3], ]$subclone <-
  names(subclones)[3]
VDJ.IGH.sub[!VDJ.IGH.sub$subclone %in% names(subclones), ]$subclone <-
  "others"
table(VDJ.IGH.sub$subclone)

VDJ.IGH.sub$v_call_sim <- factor(VDJ.IGH.sub$v_call_sim,
                                 levels = names(sort(
                                   table(VDJ.IGH.sub$v_call_sim),
                                   decreasing = T
                                 )))
VDJ.IGH.sub$d_call_sim <- factor(VDJ.IGH.sub$d_call_sim,
                                 levels = names(sort(
                                   table(VDJ.IGH.sub$d_call_sim),
                                   decreasing = T
                                 )))
VDJ.IGH.sub$j_call_sim <- factor(VDJ.IGH.sub$j_call_sim,
                                 levels = names(sort(
                                   table(VDJ.IGH.sub$j_call_sim),
                                   decreasing = T
                                 )))
...
plot.data$node <-
  factor(plot.data$node, levels = rev(c(
    levels(VDJ.IGH.sub$v_call_sim),
...
temp <-
  color.subclone[rep(c("Subclone_1", "Subclone_2", "Subclone_3", "others"),
                     time = c(3, 3, 3, 4))]
...
ggplot(
  ...
  geom_sankey_label(size = 3,
                    ...
                    fill = "gray30") +
  scale_x_discrete(
    breaks = c("v_call_sim", "d_call_sim", "j_call_sim"),
...
## 5.1. constant region ----------------------------------------------------
TR_C_genes <-
  sort(gene.info.hg38[gene.info.hg38$`Gene type` == "TR_C_gene", ]$`Gene name`)
...
for (one in cell.list) {
  if (file.exists(paste0("data/tcr/", one, ".IgBlast.TRB.tsv"))) {
    temp <-
      read.table(
        paste0("data/tcr/", one, ".IgBlast.TRB.tsv"),
        header = T,
        sep = "\t"
      )
    #temp <- VDJFilter(temp)
  ...
  if (file.exists(paste0("data/tcr/", one, ".IgBlast.TRA.tsv"))) {
    temp <-
      read.table(
        paste0("data/tcr/", one, ".IgBlast.TRA.tsv"),
        header = T,
        sep = "\t"
      )
    #temp <- VDJFilter(temp)
...
VDJ.TRB[(nrow(VDJ.TRB) + 1):(nrow(VDJ.TRB) + length(empty)), ] <-
  "none"
VDJ.TRB[(nrow(VDJ.TRB) - length(empty) + 1):(nrow(VDJ.TRB)), ]$Cell <-
  empty
empty <- cell.list[!cell.list %in% VDJ.TRA$Cell]
VDJ.TRA[(nrow(VDJ.TRA) + 1):(nrow(VDJ.TRA) + length(empty)), ] <-
  "none"
VDJ.TRA[(nrow(VDJ.TRA) - length(empty) + 1):(nrow(VDJ.TRA)), ]$Cell <-
  empty
...
colnames(TCR.stat) <-
  c("Cell", "TRA_Reads", "TRB_Reads", "TRA_Cluster", "TRB_Cluster")
rownames(TCR.stat) <- TCR.stat$Cell
identical(TCR.stat$Cell, VDJ.TRA$Cell)
identical(TCR.stat$Cell, VDJ.TRB$Cell)
VDJ.TRA[TCR.stat$TRA_Reads < 5, c("v_call", "d_call", "j_call")] <-
  "none"
VDJ.TRB[TCR.stat$TRB_Reads < 5, c("v_call", "d_call", "j_call")] <-
  "none"
...
VDJ.TRA.sub[VDJ.TRA.sub$subclone == subclones[1], ]$subclone <-
  names(subclones)[1]
VDJ.TRA.sub[VDJ.TRA.sub$subclone == subclones[2], ]$subclone <-
  names(subclones)[2]
#VDJ.TRA.sub[VDJ.TRA.sub$subclone == subclones[3],]$subclone <- names(subclones)[3]
VDJ.TRA.sub[!VDJ.TRA.sub$subclone %in% names(subclones), ]$subclone <-
  "others"
table(VDJ.TRA.sub$subclone)

VDJ.TRA.sub$v_call_sim <- factor(VDJ.TRA.sub$v_call_sim,
                                 levels = names(sort(
                                   table(VDJ.TRA.sub$v_call_sim),
                                   decreasing = T
                                 )))
VDJ.TRA.sub$j_call_sim <- factor(VDJ.TRA.sub$j_call_sim,
                                 levels = names(sort(
                                   table(VDJ.TRA.sub$j_call_sim),
                                   decreasing = T
                                 )))
