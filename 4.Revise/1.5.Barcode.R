...
barcode <- readDNAStringSet("../1.1.Error_Rate/data/Barcode01_96.fa", format="fasta",
                            seek.first.rec=FALSE, use.names=T)

adist(barcode$Barcode01, barcode$Barcode02, partial = F, ignore.case = T)
...
while (length(barcode_plus) < n_ideal) {
  ...
  min_dist <- lapply(barcode_plus, function(x){return(as.numeric(adist(x, seq)))}) %>%
    unlist %>% min
  
  if(min_dist >= cutoff.dist){
    barcode_plus[[paste("Barcode",id, sep = "")]] <- seq
    id <- id + 1
    message(paste0(Sys.time(),": ", id, " barcodes identified, ", 
                   times, " barcodes tried in this round."))
    times <- 0
  }
}
