...
for (cell in rownames(sample.info)) {
  counts <- read.table(paste0("data/",cell,"_stat.summary.txt"), row.names = 1) %>%
    t %>% as.data.frame
  counts$Cell <- cell
  counts$Stage <- c("Demultiplexing", "Quality_control", "Deduplication")
...
names(color.stage) <- c("Total","Demultiplexing", "Quality_control", "Deduplication")

## 2.1. Boxplot of single-cell statistics -----

colnames(reads.stat)
plot.data <- melt(reads.stat[reads.stat$Cell %in% rownames(sample.info),c(1:4,6,8:11)],
                  id.vars = c("Cell", "Stage"))
plot.data$Stage <- factor(plot.data$Stage,
                          levels = c("Demultiplexing", "Quality_control", "Deduplication"))
plot.data$variable <- factor(plot.data$variable,
                          levels = c("log10_Number_of_reads", "log10_Total_bases", "Read_length_N50",
                                     "Mean_read_length", "Median_read_length","Mean_read_quality" ,"Median_read_quality"))
...
names(color.stage) <- c("PromethION_output","Demultiplexing", "Quality_control", "Deduplication")

plot.data <- data.frame("Stage" = c("PromethION_output", "Demultiplexing",
                                    "Quality_control", "Deduplication"))
plot.data$Number_of_Reads <- c(84801426,aggregate(reads.stat$Number_of_reads,
                                                  list(reads.stat$Stage),
                                                  sum)$x)
plot.data$Total_bases <- c(106795866640,aggregate(reads.stat$Total_bases,
                                                  list(reads.stat$Stage),
                                                  sum)$x)
test <- data.frame("Stage" = paste("test",1:3),
                   "Number_of_Reads" = NA,
                   "Total_bases" = NA)
plot.data <- rbind(plot.data,test)

plot.data$Stage <- factor(plot.data$Stage, levels = rev(c("PromethION_output", "Demultiplexing",
                                                      "Quality_control", "Deduplication",test$Stage)))

pdf("Circle.Reads.Bases.pdf", 8,6)
multiplot(
ggplot(plot.data, aes(x = Stage, y = Number_of_Reads / 1e6, fill = Stage))+
  geom_bar(stat = "identity")+
  coord_polar(theta="y",start=0,direction=1) +
  ylim(c(0, max(plot.data$Number_of_Reads))) + 
...
ggplot(plot.data, aes(x = Stage, y = Total_bases / 1e9, fill = Stage))+
  geom_bar(stat = "identity")+
  coord_polar(theta="y",start=0,direction=1) +
  ylim(c(0, max(plot.data$Total_bases))) + 
  scale_fill_manual(values = color.stage)+
