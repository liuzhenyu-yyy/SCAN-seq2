...
ReadGeneInfo("mm10")
gtf <- import_gtf("E:/LabWork/genome/Homo_sapiens.GRCh38.101.gtf",
                  feature_type=NULL, out_df=FALSE)
...
rc_trans <- readRDS("E:/LabWork/Project/SCANSeq2/batch/6 IGG/data/rc_trans.RDS")
...
obj.IGG <- CellCycleScoring(obj.IGG,
...
isoform_IGG6 <- read.table("../../batch/6 IGG/data/IGG6/Isoform.category.tsv", header = T)
isoform_IGG24 <- read.table("../../batch/6 IGG/data/IGG24/Isoform.category.tsv", header = T)
isoform_IGG48 <- read.table("../../batch/6 IGG/data/IGG48/Isoform.category.tsv", header = T)
...
multiplot(DimPlot(obj.Hela, group.by = "seurat_clusters",
...
Idents(obj.Hela)
marker.Hela <- FindAllMarkers(obj.Hela, only.pos = TRUE,
...
DoHeatmap(obj.Hela, features = marker.Hela$gene,label = T,
          draw.lines = F,
...
GO.Hela.C0 <- enrichGO(marker.Hela[marker.Hela$cluster==0,]$gene,
                  keyType = "SYMBOL",
                  OrgDb=org.Hs.eg.db, ont = "BP", 
                  pAdjustMethod = "BH", pvalueCutoff  = 0.05)
GO.Hela.C0 <- simplify(GO.Hela.C0, cutoff=0.7,
                  by="p.adjust", select_fun=min)
GO.Hela.C1 <- enrichGO(marker.Hela[marker.Hela$cluster==1,]$gene,
                       keyType = "SYMBOL",
                       OrgDb=org.Hs.eg.db, ont = "BP", 
                       pAdjustMethod = "BH", pvalueCutoff  = 0.05)
GO.Hela.C1 <- simplify(GO.Hela.C1, cutoff=0.7,
                       by="p.adjust", select_fun=min)
GO.Hela.C2 <- enrichGO(marker.Hela[marker.Hela$cluster==2,]$gene,
                       keyType = "SYMBOL",
                       OrgDb=org.Hs.eg.db, ont = "BP", 
                       pAdjustMethod = "BH", pvalueCutoff  = 0.05)
GO.Hela.C2 <- simplify(GO.Hela.C2, cutoff=0.7,
                       by="p.adjust", select_fun=min)
...
plot.data <- rbind(GO.Hela.C0@result[c("GO:0006839","GO:2000573","GO:0043462",
                                       "GO:0050821","GO:1904951","GO:0007004","GO:0002446"),],
                   GO.Hela.C1@result[c("GO:0016126","GO:0006066","GO:0009165",
                                       "GO:0006790","GO:0034976","GO:0009615","GO:0006767"),],
                   GO.Hela.C2@result[c("GO:0000184","GO:0006413","GO:0042255",
                                       "GO:0006364","GO:1903311","GO:0006457","GO:2001244"),])
plot.data$cluster <- rep(c(0,1,2), times = c(7,7,7))

frac2number <- function(x){
  temp <- strsplit(x , "/")
  temp <- lapply(temp, function(x){return(as.numeric(x[1]) / as.numeric(x[2]))})
  return(unlist(temp))
}
plot.data$Enrichment <- frac2number(plot.data$GeneRatio) / frac2number(plot.data$BgRatio)
...
ggplot(plot.data)+
  geom_bar(aes(x = Description, y = log10(Enrichment),
...
for (one in features) {
  obj.Hela@meta.data[,one] <- isoform[rownames(obj.Hela@meta.data),one]
}

pdf("Hela/Vln.Isoform.classification.pdf",10,4)
VlnPlot(obj.Hela, features = features,
        cols = color.cluster, ncol = 5)
dev.off()

###  DTU analysis----------------------------
marker.Hela.C2 <- FindMarkers(object = obj.Hela,
                              ident.1 = 2, ident.2 = c(0,1),
                              min.pct = 0.25,
...
DTU.Hela <- run_drimseq(counts = as.matrix(rc_trans.Hela),
                        tx2gene = gene2trans,
                        pd=pb, id_col = "Rename",
                        ...
                        min_feature_expr = 5)
DTU.Hela <- posthoc_and_stager(dturtle = DTU.Hela, ofdr = 0.05, posthoc = 0.1)
...
plot(Venn(list("DGE" = rownames(marker.Hela.C2),
...
temp <- plot_proportion_barplot(dturtle = DTU.Hela, genes = "TFAP2A")
temp$TFAP2A
temp <- plot_proportion_pheatmap(dturtle = DTU.Hela, genes = "TFAP2A", 
                                 sample_meta_table_columns = c("sample_id","seurat_clusters_2"),
                                 include_expression = TRUE, treeheight_col=20,
                                 BPPARAM = BiocParallel::SnowParam(1))
temp$TFAP2A
plot_transcripts_view(dturtle = DTU.Hela, 
                      genes = "TFAP2A", 
                      gtf = gtf, 
...
DTU.Hela.sig <- DTU.Hela$meta_table_tx[DTU.Hela$meta_table_tx$tx %in% 
                                         DTU.Hela$sig_tx,]
...
for (one in gene.selected) {
  ...
  temp <- data.frame(row.names = one$gene[1],
                     "Gene" = one$gene[1],
                     "Isoform_C2" = trans_C2,
...
temp <- DTU.Hela.sig.2[DTU.Hela.sig.2$Type_C2 != DTU.Hela.sig.2$Type_C0_C1,]
...
obj.HepG2 <- RunUMAP(obj.HepG2, dims = 1:5,
                    n.neighbors = 40, min.dist = 0.05)
obj.HepG2 <- FindClusters(obj.HepG2, resolution = 0.2)

pdf("HepG2/UMAP.befor.regression.pdf",8,6)
multiplot(DimPlot(obj.HepG2, group.by = "Gene_snn_res.0.2",
                  cols = color.cluster) + coord_equal(1.3),
...
Idents(obj.HepG2)
marker.HepG2 <- FindAllMarkers(obj.HepG2, only.pos = TRUE,
                              min.pct = 0.25, logfc.threshold = 0.25)
...
DoHeatmap(obj.HepG2, features = marker.HepG2$gene,label = T,
          draw.lines = F,
          group.colors = color.cluster,
          disp.min = -2.5, disp.max = 2.5,
          angle = 0)+
...
GO.HepG2.C0 <- enrichGO(marker.HepG2[marker.HepG2$cluster==0,]$gene,
                       keyType = "SYMBOL",
                       OrgDb=org.Hs.eg.db, ont = "BP", 
                       pAdjustMethod = "BH", pvalueCutoff  = 0.05)
GO.HepG2.C0 <- simplify(GO.HepG2.C0, cutoff=0.7,
                       by="p.adjust", select_fun=min)
GO.HepG2.C1 <- enrichGO(marker.HepG2[marker.HepG2$cluster==1,]$gene,
                       keyType = "SYMBOL",
                       OrgDb=org.Hs.eg.db, ont = "BP", 
                       pAdjustMethod = "BH", pvalueCutoff  = 0.05)
GO.HepG2.C1 <- simplify(GO.HepG2.C1, cutoff=0.7,
                       by="p.adjust", select_fun=min)
GO.HepG2.C2 <- enrichGO(marker.HepG2[marker.HepG2$cluster==2,]$gene,
                       keyType = "SYMBOL",
                       OrgDb=org.Hs.eg.db, ont = "BP", 
                       pAdjustMethod = "BH", pvalueCutoff  = 0.05)
GO.HepG2.C2 <- simplify(GO.HepG2.C2, cutoff=0.7,
                       by="p.adjust", select_fun=min)
...
plot.data <- rbind(GO.HepG2.C0@result[c("GO:0022613","GO:0002446","GO:0140053",
                                       "GO:0072655","GO:0050821","GO:1902903","GO:0070125"),],
                   GO.HepG2.C1@result[c("GO:0008202","GO:0006066","GO:0046395",
                                       "GO:0044282","GO:0006790","GO:1901605","GO:0098754"),],
                   GO.HepG2.C2@result[c("GO:0000184","GO:0006413","GO:0042255",
                                       "GO:0006364","GO:0022618","GO:0030968","GO:0006986"),])
plot.data$cluster <- rep(c(0,1,2), times = c(7,7,7))
...
for (one in features) {
  obj.HepG2@meta.data[,one] <- isoform[rownames(obj.HepG2@meta.data),one]
}

pdf("HepG2/Vln.Isoform.classification.pdf",10,4)
VlnPlot(obj.HepG2, features = features,
        cols = color.cluster, ncol = 5)
dev.off()

# DTU analysis
marker.HepG2.C2 <- FindMarkers(object = obj.HepG2,
                              ident.1 = 2, ident.2 = c(0,1),
                              min.pct = 0.25,
                              logfc.threshold = 0.25,
                              only.pos = F)
...
DTU.HepG2 <- run_drimseq(counts = as.matrix(rc_trans.HepG2),
                        tx2gene = gene2trans,
                        pd=pb, id_col = "Rename",
                        cond_col = "seurat_clusters",
                        cond_levels = c("C2", "C0_C1"),
...
DTU.HepG2 <- posthoc_and_stager(dturtle = DTU.HepG2, ofdr = 0.05, posthoc = 0.1)
...
plot(Venn(list("DGE" = rownames(marker.HepG2.C2),
...
temp <- plot_proportion_barplot(dturtle = DTU.HepG2, genes = "EIF4G1")
temp$EIF4G1
temp <- plot_proportion_pheatmap(dturtle = DTU.HepG2, genes = "EIF4G1", 
                                 sample_meta_table_columns = c("sample_id","seurat_clusters_2"),
                                 include_expression = TRUE, treeheight_col=20,
                                 BPPARAM = BiocParallel::SnowParam(1))
temp$EIF4G1
plot_transcripts_view(dturtle = DTU.HepG2, 
                      genes = "EIF4G1", 
                      gtf = gtf, 
                      genome = NULL, reduce_introns_min_size = 100,
                      arrow_colors = c("#f46d43","#74add1"),
                      one_to_one = TRUE,
                      savepath = "./HepG2",filename_ext  = "_transcripts.pdf",
                      width = 5, height = 3)
...
## DEG and DTU genes --------------------------
plot(Venn(list("Hela C0" = marker.Hela[marker.Hela$cluster==0,]$gene,
               "HepG2 C0" = marker.HepG2[marker.HepG2$cluster==0,]$gene)))
plot(Venn(list("Hela C1" = marker.Hela[marker.Hela$cluster==1,]$gene,
               "HepG2 C1" = marker.HepG2[marker.HepG2$cluster==1,]$gene)))
plot(Venn(list("Hela C2" = marker.Hela[marker.Hela$cluster==2,]$gene,
               "HepG2 C2" = marker.HepG2[marker.HepG2$cluster==2,]$gene)))

plot(Venn(list("Hela C2" = rownames(marker.Hela.C2)[marker.Hela.C2$avg_log2FC>0],
               "HepG2 C2" = rownames(marker.HepG2.C2)[marker.HepG2.C2$avg_log2FC>0])))
...
#      doWeights = TRUE,
#      show = list( SetLabels = T, Faces = FALSE))
plot(Venn(list("Hela" = rownames(marker.Hela.C2)[marker.Hela.C2$avg_log2FC>0],
               "HepG2" = rownames(marker.HepG2.C2)[marker.HepG2.C2$avg_log2FC>0])),
     doWeights = TRUE,
     show = list( SetLabels = T, Faces = FALSE))
plot(Venn(list("Hela" = rownames(marker.Hela.C2)[marker.Hela.C2$avg_log2FC<0],
               "HepG2" = rownames(marker.HepG2.C2)[marker.HepG2.C2$avg_log2FC<0])),
...
DTU.up.Hela <- DTU.Hela$sig_tx[DTU.Hela$meta_table_tx[DTU.Hela$sig_tx,]$exp_in_C2 
                               > DTU.Hela$meta_table_tx[DTU.Hela$sig_tx,]$exp_in_C0_C1]
DTU.down.Hela <- DTU.Hela$sig_tx[DTU.Hela$meta_table_tx[DTU.Hela$sig_tx,]$exp_in_C2 
                                 < DTU.Hela$meta_table_tx[DTU.Hela$sig_tx,]$exp_in_C0_C1]
DTU.up.HepG2 <- DTU.HepG2$sig_tx[DTU.HepG2$meta_table_tx[DTU.HepG2$sig_tx,]$exp_in_C2 
                               > DTU.HepG2$meta_table_tx[DTU.HepG2$sig_tx,]$exp_in_C0_C1]
DTU.down.HepG2 <- DTU.HepG2$sig_tx[DTU.HepG2$meta_table_tx[DTU.HepG2$sig_tx,]$exp_in_C2 
                                 < DTU.HepG2$meta_table_tx[DTU.HepG2$sig_tx,]$exp_in_C0_C1]
...
plot(Venn(list("DEG" = rownames(marker.Hela.C2),
               "DTU" = DTU.Hela$sig_gene)),
     doWeights = TRUE,
     show = list( SetLabels = T, Faces = FALSE))
plot(Venn(list("DEG" = rownames(marker.HepG2.C2),
               "DTU" = DTU.HepG2$sig_gene)),
     doWeights = TRUE,
     show = list( SetLabels = T, Faces = FALSE))
...
plot.data.count <- data.frame(row.names = common.term,
                              ...
                              "HepG2_1"= 1000,
                              "HepG2_2" = 1000)
plot.data.OR <- data.frame(row.names = common.term,
                           "Decription" = GO.Hela.C2@result[common.term,]$Description,
                           "Hela_0"= 1000,
                           "Hela_1"= 1000,
                           "Hela_2" = 1000,
...
for (one in common.term) {
  gene.one <- unique(select(org.Hs.eg.db, 
...
common.term <- common.term[order(plot.data.OR$Hela_2, decreasing = T)]
...
ggplot(plot.data)+
  geom_point(aes(x = paste0("C",Cluster_1), y = Description,
                 size = Count, color = log10(Enrichment+0.1)))+
...
plot.data <- plot.data[,c("Cell_Line","seurat_clusters","percent.NIC.CJ","percent.NIC.CS","percent.NIC.IR")]
colnames(plot.data)[3:5] <- c("CJ","CS","IR")
plot.data <- melt(plot.data)
plot.data$Seurat_Clusters <- as.numeric(unfactor(plot.data$seurat_clusters)) + 1
plot.data$Seurat_Clusters <- paste("C",plot.data$Seurat_Clusters,sep = "")
...
ggplot(plot.data,aes(x = Seurat_Clusters, y = value * 100))+
  ...
  geom_jitter(width = 0.3,height = 0,size = 0.00001)+
  scale_fill_manual(values = color.cluster)+
  facet_grid(rows = vars(variable),
             cols = vars(Cell_Line))+
  ggsignif::geom_signif(comparisons = list(c("C3", "C1"),
                                           c("C3", "C2")),
                        map_signif_level=T,
                        textsize=1.5,
                        test=wilcox.test,
...
plot_transcripts_view(dturtle = DTU.Hela, genes = CC.gene, 
                      gtf = gtf, genome = NULL, reduce_introns_min_size = 100,
                      arrow_colors = c("#f46d43","#74add1"),
                      one_to_one = TRUE,
                      savepath = "./Compare_Cell_Line/CellCycle",filename_ext  = "_Hela.transcripts.pdf",
                      width = 5, height = 3)
plot_transcripts_view(dturtle = DTU.HepG2, genes = CC.gene, 
                      gtf = gtf, genome = NULL, reduce_introns_min_size = 100,
                      arrow_colors = c("#f46d43","#74add1"),
                      one_to_one = TRUE,
                      savepath = "./Compare_Cell_Line/CellCycle",filename_ext  = "_HepG2.transcripts.pdf",
                      width = 5, height = 3)

Liver_cancer_gene <- read.table("frequently-mutated-genes.2022-04-14.tsv",
                                sep = "\t",
                                header = T)
liver_gene_selected <- intersect(DTU.HepG2$sig_gene,
                                 Liver_cancer_gene[1:100,]$Symbol)
plot_transcripts_view(dturtle = DTU.HepG2, genes = liver_gene_selected, 
                      gtf = gtf, genome = NULL, reduce_introns_min_size = 100,
                      arrow_colors = c("#f46d43","#74add1"),
                      one_to_one = TRUE,
                      savepath = "./Compare_Cell_Line",filename_ext  = "_HepG2.transcripts.pdf",
                      width = 5, height = 3)
...
sample.info.Hela$seurat_clusters <- paste("C",as.numeric(unfactor(sample.info.Hela$seurat_clusters))+1,
                                          sep = "")
sample.info.HepG2$seurat_clusters <- paste("C",as.numeric(unfactor(sample.info.HepG2$seurat_clusters))+1,
                                          sep = "")
...
marker.Hela$cluster <- paste("C",as.numeric(unfactor(marker.Hela$cluster))+1,
                             sep = "")
marker.HepG2$cluster <- paste("C",as.numeric(unfactor(marker.HepG2$cluster))+1,
                             sep = "")
