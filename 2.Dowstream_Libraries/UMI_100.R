setwd("E:/LabWork/Project/SCANSeq2/batch/4 UMI_100/")
...
sample.info$Library <- "UMI_100"
sample.info$Name <- rownames(sample.info)
sample.info$Rename <- paste("UMI_100", sample.info$Name, sep = "_")
...
quantile(sample.info$Mapped_Reads) # 253786.5
median(sample.info[sample.info$Organism == "Human", ]$Gene_Detected) # 8032
median(sample.info[sample.info$Organism == "Mouse", ]$Trans_Detected) # 8188.5

mean(sample.info[sample.info$Organism == "Human", ]$Gene_Detected) # 7748.521
mean(sample.info[sample.info$Organism == "Mouse", ]$Trans_Detected) # 8300.062
...
median(sample.info[sample.info$Pass_QC == 1 &
                     sample.info$Organism == "Human", ]$Gene_Detected) # 8551
median(sample.info[sample.info$Pass_QC == 1 &
                     sample.info$Organism == "Human", ]$Trans_Detected) # 11349

median(sample.info[sample.info$Pass_QC == 1 &
                     sample.info$Organism == "Mouse", ]$Gene_Detected) # 7188
median(sample.info[sample.info$Pass_QC == 1 &
                     sample.info$Organism == "Mouse", ]$Trans_Detected) # 8613.5
...
pdf("Hist_Ratio_Human.cDNA.pdf", 4.5, 2)
...
ggplot() +
  geom_point(aes(
    ...
    y = log10(1 + rc_ERCC$UMI_100_Bc75_Bc01),
    color = "Bc75_Bc01"
  )) +
  geom_point(aes(
    x = log10(1 + ERCC.info$concentration_mix_1),
    y = log10(1 + rc_ERCC$`UMI_100_Bc75_Bc02`),
    color = "Bc75_Bc02"
  )) +
  geom_point(aes(
    x = log10(1 + ERCC.info$concentration_mix_1),
    y = log10(1 + rc_ERCC$`UMI_100_Bc75_Bc03`),
    color = "Bc75_Bc03"
  )) +
  geom_point(aes(
    x = log10(1 + ERCC.info$concentration_mix_1),
    y = log10(1 + rc_ERCC$`UMI_100_Bc75_Bc04`),
    color = "Bc75_Bc04"
  )) +
...
write.table(sample.info,
            ...
            quote = F,
            sep = "\t")
saveRDS(mix.obj, "obj.UMI_100.rds")
