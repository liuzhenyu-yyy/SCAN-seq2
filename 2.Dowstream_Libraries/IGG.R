...
# IGG6
sample.info.IGG6 <-
  read.table("data/IGG6/summary.txt",
             header = T,
             row.names = 1)

sample.info.IGG6$Library <- "IGG6"
sample.info.IGG6$Name <- rownames(sample.info.IGG6)
sample.info.IGG6$Rename <-
  paste("IGG6", sample.info.IGG6$Name, sep = "_")
...
sample.info.IGG6[sample.info.IGG6$Barcode.5 %in% as.character(57:68), ]$Cell_Line <-
  "Hela"
sample.info.IGG6[sample.info.IGG6$Barcode.5 %in% as.character(69:80), ]$Cell_Line <-
  "HepG2"
...
# IGG24
sample.info.IGG24 <-
  read.table("data/IGG24/summary.txt",
             header = T,
             row.names = 1)

sample.info.IGG24$Library <- "IGG24"
sample.info.IGG24$Name <- rownames(sample.info.IGG24)
sample.info.IGG24$Rename <-
  paste("IGG24", sample.info.IGG24$Name, sep = "_")
...
sample.info.IGG24[sample.info.IGG24$Barcode.5 %in% as.character(63:74), ]$Cell_Line <-
  "Hela"
sample.info.IGG24[sample.info.IGG24$Barcode.5 %in% as.character(51:62), ]$Cell_Line <-
  "HepG2"
...
# IGG48
sample.info.IGG48 <-
  read.table("data/IGG48/summary.txt",
             header = T,
             row.names = 1)

sample.info.IGG48$Library <- "IGG48"
sample.info.IGG48$Name <- rownames(sample.info.IGG48)
sample.info.IGG48$Rename <-
  paste("IGG48", sample.info.IGG48$Name, sep = "_")
...
sample.info.IGG48[sample.info.IGG48$Barcode.5 %in% as.character(c(51:56)), ]$Time <-
  6
sample.info.IGG48[sample.info.IGG48$Barcode.5 %in% as.character(c(75:80)), ]$Time <-
  24
...
# IGG6
rc_gene_IGG6 <-
  as.data.frame(fread("data/IGG6/Gene.Count.txt", header = T))
rc_trans_IGG6 <-
  as.data.frame(fread("data/IGG6/Trans.Count.txt", header = T))
...
# IGG24
rc_gene_IGG24 <-
  as.data.frame(fread("data/IGG24/Gene.Count.txt", header = T))
rc_trans_IGG24 <-
  as.data.frame(fread("data/IGG24/Trans.Count.txt", header = T))
...
# IGG48
rc_gene_IGG48 <-
  as.data.frame(fread("data/IGG48/Gene.Count.txt", header = T))
rc_trans_IGG48 <-
  as.data.frame(fread("data/IGG48/Trans.Count.txt", header = T))
...
sample.info$Gene_Detected <-
  colSums(rc_gene[, rownames(sample.info)] >= 1)
sample.info$Trans_Detected <-
  colSums(rc_trans[, rownames(sample.info)] >= 1)
...
ggplot(sample.info) +
  ...
  geom_boxplot(
    aes(x = Time, y = Mapped_percent, color = Cell_Line),
    size = 0.4,
    alpha = 0,
    show.legend = F
  ) +
...
ggplot(sample.info) +
  ...
  geom_boxplot(
    aes(x = Time, y = Gene_Detected, color = Cell_Line),
    size = 0.4,
    alpha = 0,
    show.legend = F
  ) +
...
ggplot(sample.info) +
  ...
  geom_boxplot(
    aes(x = Time, y = Trans_Detected, color = Cell_Line),
    size = 0.4,
    alpha = 0,
    show.legend = F
  ) +
...
aggregate(
  sample.info$Raw_Reads,
  list("Time" = sample.info$Time,
       "Cell" = sample.info$Cell_Line),
  FUN = median
)
aggregate(
  sample.info$Mapped_percent,
...
rc_gene <-
  rc_gene[rownames(rc_gene) %in% gene.info.hg38$`Gene stable ID`, ]
rc_gene <-
  rc_gene[!duplicated(gene.info.hg38[rownames(rc_gene), ]$`Gene name`), ]
...
mix.obj <-
  subset(mix.obj, subset = Gene_Detected > 3500 & percent.mt < 15)
...
ggplot() +
  geom_point(aes(
    ...
    y = log10(1 + rc_ERCC$IGG6_Bc57_Bc01),
    color = "IGG6_Bc57_Bc01"
  )) +
  geom_point(aes(
    x = log10(1 + ERCC.info$concentration_mix_1),
    y = log10(1 + rc_ERCC$IGG6_Bc61_Bc15),
    color = "IGG6_Bc61_Bc15"
  )) +
  geom_point(aes(
    x = log10(1 + ERCC.info$concentration_mix_1),
    y = log10(1 + rc_ERCC$IGG6_Bc57_Bc24),
    color = "IGG6_Bc57_Bc24"
  )) +
  geom_point(aes(
    x = log10(1 + ERCC.info$concentration_mix_1),
    y = log10(1 + rc_ERCC$IGG6_Bc59_Bc31),
    color = "IGG6_Bc59_Bc31"
  )) +
