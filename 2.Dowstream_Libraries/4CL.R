setwd("E:/LabWork/Project/SCANSeq2/batch/3 4CL/")
...
sample.info$Library <- "4CL"
sample.info$Name <- rownames(sample.info)
sample.info$Rename <- paste("4CL", sample.info$Name, sep = "_")
...
sample.info[(
  as.numeric(sample.info$Barcode.5) %in% (c(54:56, 72:74, 75, 76)) &
    as.numeric(sample.info$Barcode.3) %in% (c(01:16))
) |
  (
    as.numeric(sample.info$Barcode.5) %in% (c(51:53, 69:71, 78)) &
      as.numeric(sample.info$Barcode.3) %in% (c(17:32))
  ), ]$Cell_Line <- "293T"
sample.info[(
  as.numeric(sample.info$Barcode.5) %in% (c(51:53, 63:65, 78, 79)) &
    as.numeric(sample.info$Barcode.3) %in% (c(01:16))
) |
  (
    as.numeric(sample.info$Barcode.5) %in% (c(54:56, 66:68, 80, 75)) &
      as.numeric(sample.info$Barcode.3) %in% (c(17:32))
  ), ]$Cell_Line <- "3T3"
sample.info[(
  as.numeric(sample.info$Barcode.5) %in% (c(57:59, 69:71, 80)) &
    as.numeric(sample.info$Barcode.3) %in% (c(01:16))
) |
  (
    as.numeric(sample.info$Barcode.5) %in% (c(60:62, 72:74, 76, 77)) &
      as.numeric(sample.info$Barcode.3) %in% (c(17:32))
  ), ]$Cell_Line <- "MEF"
sample.info[(
  as.numeric(sample.info$Barcode.5) %in% (c(60:62, 66:68, 77)) &
    as.numeric(sample.info$Barcode.3) %in% (c(01:16))
) |
  (
    as.numeric(sample.info$Barcode.5) %in% (c(57:59, 63:65, 79, 80)) &
      as.numeric(sample.info$Barcode.3) %in% (c(17:32))
  ), ]$Cell_Line <- "K562"
table(sample.info$Cell_Line)

sample.info$Cell_Line <- factor(
  sample.info$Cell_Line,
  levels = c(
    "GM12878",
    "HepG2",
    "Hela",
...
sample.info.human$Gene_Detected <-
  colSums(rc_gene_human[, human.cell] >= 1)
sample.info.human$Trans_Detected <-
  colSums(rc_trans_human[, human.cell] >= 1)
sample.info.mouse$Gene_Detected <-
  colSums(rc_gene_mouse[, mouse.cell] >= 1)
sample.info.mouse$Trans_Detected <-
  colSums(rc_trans_mouse[, mouse.cell] >= 1)
...
quantile(sample.info$Mapped_Reads) # 58651.50
median(sample.info[sample.info$Organism == "Human", ]$Gene_Detected) # 4919.5
median(sample.info[sample.info$Organism == "Mouse", ]$Trans_Detected) # 4882

mean(sample.info[sample.info$Organism == "Human", ]$Gene_Detected) # 4216.5
mean(sample.info[sample.info$Organism == "Mouse", ]$Trans_Detected) # 4338.508

aggregate(sample.info$Raw_Reads, list(sample.info$Cell_Line), FUN = mean)
aggregate(sample.info$Mapped_percent,
          list(sample.info$Cell_Line),
          FUN = mean)
aggregate(sample.info$Gene_Detected, list(sample.info$Cell_Line), FUN =
            mean)
aggregate(sample.info$Trans_Detected,
          list(sample.info$Cell_Line),
          FUN = mean)
...
FeatureScatter(mix.obj,
               feature1 = "Gene_Detected",
               feature2 = "percent.mt",
               cols = mycolor) +
...
median(sample.info[sample.info$Pass_QC == 1 &
                     sample.info$Organism == "Human", ]$Gene_Detected) # 5422.5
median(sample.info[sample.info$Pass_QC == 1 &
                     sample.info$Organism == "Human", ]$Trans_Detected) # 6441.5

median(sample.info[sample.info$Pass_QC == 1 &
                     sample.info$Organism == "Mouse", ]$Gene_Detected) # 4755
median(sample.info[sample.info$Pass_QC == 1 &
                     sample.info$Organism == "Mouse", ]$Trans_Detected) # 5272
...
top5 <-
  marker.all %>% group_by(cluster) %>% top_n(n = 5, wt = avg_log2FC)
...
pdf("Vln.marker.cluster.pdf", 20, 15)
VlnPlot(mix.obj, features = marker.list, cols = mycolor)
dev.off()
...
sum(Barnyard.data[human.cell, ]$human_transcripts) /
  sum(Barnyard.data[human.cell, c("human_transcripts", "mouse_transcripts")]) # 0.9757776
sum(Barnyard.data[mouse.cell, ]$mouse_transcripts) /
  sum(Barnyard.data[mouse.cell, c("human_transcripts", "mouse_transcripts")]) # 0.9596069

# 6. Correlation Among Cells ----------------------------------------------
library(pheatmap)

anno <- data.frame(
  row.names = rownames(sample.info),
...
pheatmap(
  cor.gene[rownames(cor.gene) %in% human.cell,
           colnames(cor.gene) %in% human.cell],
  main = "Correlation at Gene Level",
  cluster_rows = F,
  cluster_cols = F,
  annotation_row = anno,
...
pheatmap(
  cor.gene[rownames(cor.gene) %in% mouse.cell,
           colnames(cor.gene) %in% mouse.cell],
  main = "Correlation at Gene Level",
  cluster_rows = F,
  cluster_cols = F,
  annotation_row = anno,
...
for (one in unique(sample.info.qc$Cell_Line)) {
  temp <- data.frame("cell_line" = one,
                     "Cor" = as.numeric(cor.gene[sample.info.qc[sample.info.qc$Cell_Line ==
                                                                  one, ]$Rename,
                                                 sample.info.qc[sample.info.qc$Cell_Line ==
                                                                  one, ]$Rename]))
...
ggplot() +
  geom_point(aes(
    ...
    y = log10(1 + rc_ERCC$`4CL_Bc51_Bc01`),
    color = "Cell1"
  )) +
  geom_point(aes(
    x = log10(1 + ERCC.info$concentration_mix_1),
    y = log10(1 + rc_ERCC$`4CL_Bc51_Bc07`),
    color = "Cell2"
  )) +
  geom_point(aes(
    x = log10(1 + ERCC.info$concentration_mix_1),
    y = log10(1 + rc_ERCC$`4CL_Bc51_Bc09`),
    color = "Cell3"
  )) +
  geom_point(aes(
    x = log10(1 + ERCC.info$concentration_mix_1),
    y = log10(1 + rc_ERCC$`4CL_Bc51_Bc15`),
    color = "Cell4"
  )) +
...
sample.info.qc <- sample.info[sample.info$Pass_QC == 1, ]
for (one in levels(sample.info.qc$Cell_Line)) {
  write.table(
    sample.info.qc[sample.info.qc$Cell_Line == one, ]$Name,
    paste0("sample.list.4CL.", one),
    quote = F,
    sep = "\t",
    row.names = F,
    col.names = F
