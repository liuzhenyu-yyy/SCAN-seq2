setwd("E:/LabWork/Project/SCANSeq2/batch/5 UMI_200/")
...
sample.info$Library <- "UMI_200"
sample.info$Name <- rownames(sample.info)
sample.info$Rename <- paste("UMI_200", sample.info$Name, sep = "_")
...
sample.info[!sample.info$Barcode.3 %in% as.character(17:32), ]$Cell_Line <-
  "K562"
sample.info[sample.info$Barcode.3 %in% as.character(17:32), ]$Cell_Line <-
  "3T3"
...
quantile(sample.info$Mapped_Reads) # 130213.50
median(sample.info[sample.info$Organism == "Human", ]$Gene_Detected) # 7703.5
median(sample.info[sample.info$Organism == "Mouse", ]$Trans_Detected) # 8343

mean(sample.info[sample.info$Organism == "Human", ]$Gene_Detected) # 7217.365
mean(sample.info[sample.info$Organism == "Mouse", ]$Trans_Detected) # 7614.531
...
median(sample.info[sample.info$Pass_QC == 1 &
                     sample.info$Organism == "Human", ]$Gene_Detected) # 7703.5
median(sample.info[sample.info$Pass_QC == 1 &
                     sample.info$Organism == "Human", ]$Trans_Detected) # 9679

median(sample.info[sample.info$Pass_QC == 1 &
                     sample.info$Organism == "Mouse", ]$Gene_Detected) # 7039
median(sample.info[sample.info$Pass_QC == 1 &
                     sample.info$Organism == "Mouse", ]$Trans_Detected) # 8343
...
DimPlot(
  ...
  coord_equal(0.7) + NoLegend()
dev.off()

pdf("UMAP.Quality.pdf", 4.5, 1.5)
FeaturePlot(
  mix.obj,
  "nFeature_Gene",
  reduction = "umap",
...
pdf("Vln.marker.cluster.pdf", 40, 40)
...
pdf("Hist_Ratio_Human.cDNA.pdf", 5, 2.5)
...
sum(Barnyard.data[human.cell, ]$human_transcripts) /
  sum(Barnyard.data[human.cell, c("human_transcripts", "mouse_transcripts")]) # 0.974756
sum(Barnyard.data[mouse.cell, ]$mouse_transcripts) /
  sum(Barnyard.data[mouse.cell, c("human_transcripts", "mouse_transcripts")]) # 0.9495133
...
ggplot() +
  geom_point(aes(
    ...
    y = log10(1 + rc_ERCC$UMI_200_Bc67_Bc01),
    color = "Bc67_Bc01"
  )) +
  geom_point(aes(
    x = log10(1 + ERCC.info$concentration_mix_1),
    y = log10(1 + rc_ERCC$UMI_200_Bc67_Bc02),
    color = "Bc67_Bc02"
  )) +
  geom_point(aes(
    x = log10(1 + ERCC.info$concentration_mix_1),
    y = log10(1 + rc_ERCC$UMI_200_Bc67_Bc03),
    color = "Bc67_Bc03"
  )) +
  geom_point(aes(
    x = log10(1 + ERCC.info$concentration_mix_1),
    y = log10(1 + rc_ERCC$UMI_200_Bc67_Bc04),
    color = "Bc67_Bc04"
  )) +
...
write.table(sample.info,
            ...
            quote = F,
            sep = "\t")
saveRDS(mix.obj, "obj.UMI_200.rds")
